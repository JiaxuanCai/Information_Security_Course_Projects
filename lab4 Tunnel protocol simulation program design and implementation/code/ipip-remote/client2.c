...
*/
mac_addr my_mac = {0x30, 0x31, 0x32, 0x33, 0x34, 0x35};  // 本机MAC地址
// mac_addr my_mac;
...
mac_addr dst_mac = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25};
int sock_raw_fd;
...
// char myipaddr_c[] = "172.17.37.100";  // 本机IP地址
// char myipaddr_c[] = "39.97.114.89";  // 本机IP地址
char myipaddr_c[] = "172.17.37.55";
char dstip_c[] = "192.168.0.1";
...
void read_from_network_layer(int nbyte, char *packet, struct myiphdr *iniphdr,
    ...
    if (ntohs(inudphdr.dport) != 22222) return;
...
int read_from_datalink_layer(char packet[], int nbyte) {
    ...
    // if (srcip_c[0] != '2' || srcip_c[1] != '1' || srcip_c[2] != '8') return 0;
    ...
    if ((frag_off & 0x2000) || frag_off & 0x1FFF) {
        printf("fragoff %x\n", frag_off);
        ...
        // printf("read from network klayer\n")
