...
*/
mac_addr my_mac = {0x00,0x01,0x02,0x03,0x04,0x05};  // 本机MAC地址
...
mac_addr dst_mac = {0x00,0x0C,0x29,0xF6,0x50,0xcc};
//mac_addr dst_mac = {0x86, 0x8c, 0x8d, 0xee, 0xfc, 0x64};
// mac_addr dst_mac = {0x00, 0x50,0x56,0xf3,0x38,0x0f};
//mac_addr dst_mac = {0x10, 0x11, 0x12, 0x13, 0x14, 0x15};
// char dstip_c[] = "172.17.37.128";
// char dstip_c[] = "39.97.114.89";
 char dstip_c[] = "172.17.37.55";
...
char myipaddr_c[] = "172.20.10.5";  // 本机IP地址
...
ushort calc_check_udphdr(char *udp_datagram, uint udplen, uint srcip,
                         uint dstip) {
    ...
    memcpy(tmp + 12, udp_datagram, udplen);
...
void send_to_datalink_layer(char data[], int data_len, int error) {
    ...
    // 输出frame 测试
    // TODO TEST
    uchar info[] = {0x86, 0x8c, 0x8d , 0xee , 0xfc , 0x64 , 0x00 , 0x01 , 0x02 , 0x03 , 0x04, 0x05 , 0x08 , 0x00 , 0x45 , 0x00 , 0x00 , 0x1c , 0x0e , 0x87 , 0x40 , 0x00 , 0x40 , 0x11 , 0xdc , 0x76 , 0xac , 0x14 , 0x0a , 0x05 , 0x27 , 0x61 , 0x72 , 0x59 , 0xa7 , 0xcf , 0x56 , 0xce , 0x00 , 0x08 , 0xb1 , 0x6c, 00, 00, 00 ,00, 00 ,00 ,00 ,00 ,00 ,00 ,00 ,00 ,00, 00, 00, 00 ,00 ,00};
    // memcpy(&frame, &info, sizeof(info));
