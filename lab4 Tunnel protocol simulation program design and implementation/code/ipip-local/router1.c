...
// mac_addr my_mac = {0x38, 0x68, 0x93, 0xb5, 0x79, 0x35};//  = {0x10, 0x11,
// 0x12, 0x13, 0x14, 0x15};  // 本机MAC地址 mac_addr my_mac;

mac_addr my_mac;
// mac_addr dst_mac = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};  // 广播src
// MAC地址
// mac_addr dst_mac = {0x20, 0x21, 0x22, 0x23, 0x24, 0x25};  // dst MAC 地址
...
char tunnel_myipaddr_c[] = "172.20.10.5";  // 本机IP
// char tunnel_dstip_c[] = "192.168.41.128";

// 86:8c:8d:ee:fc:64
mac_addr dst_mac = {0x86, 0x8c, 0x8d, 0xee, 0xfc, 0x64};
char tunnel_dstip_c[] = "39.97.114.89";
...
void make_ipip_tunnel(uchar protocal, uint payload_len, uchar *payload,
    ...
    if (dont_frag || payload_len + IP_HDR_LEN <= OUTER_MTU) {
        ...
        // send_to_tansport_layer(sport, dport, data, strlen(data));
        send_to_tansport_layer(15000, 9999, ip_packet, ntohs(iphdr.tot_len));
        // send_to_datalink_layer(ip_packet, ntohs(iphdr.tot_len), ERROR_TAKEN);
    } else {
        // 首先确定需要分几片
        // 第一片带有IP hdr和 UDP hdr，后面的片没有 UDP hdr
...
void recv_data() {
    ...
    while (1) {
        ...
        // 这里为了测试，自定义协议protocol为过滤，只监听此协议的MAC帧
        ...
        int check_res = 0;  // frame_crc_check(cas_rec, buffer_in, nbyte);
